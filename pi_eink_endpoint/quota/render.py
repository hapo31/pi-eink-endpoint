"""Shared monochrome quota screens for the 296 x 128 panel."""
...
def _paste_icon(image: Image.Image, icon_name: str | None):
    # The XBM is deliberately bundled rather than fetched during a display update.
    if icon_name is None:
        return
    with Image.open(ASSETS / icon_name) as source:
...
def render_quota(quota: Quota | None, timezone_name: str, *, title: str = "QUOTA", icon_name: str | None = None, error: str | None = None) -> Image.Image:
    """Render a quota snapshot; unavailable values remain visibly unavailable."""
    image = Image.new("1", DISPLAY_SIZE, 1)
    draw = ImageDraw.Draw(image)
    _paste_icon(image, icon_name)
    _text(draw, (30 if icon_name else 6, 6), title, 15)
...
def render_login(verification_url: str | None, user_code: str | None = None, *, title: str = "LOGIN", icon_name: str | None = None, error: str | None = None) -> Image.Image:
    """Render an integer-module QR code with a readable manual fallback."""
    image = Image.new("1", DISPLAY_SIZE, 1)
    draw = ImageDraw.Draw(image)
    _paste_icon(image, icon_name)
    _text(draw, (30 if icon_name else 6, 7), title + " LOGIN", 14)
    ...
    _text(draw, (124, 72), "Then enter code:" if user_code else "Complete login", 10)
    if user_code:
